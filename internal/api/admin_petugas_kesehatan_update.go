...
// @Description - Updates pengguna account (email and optionally password)
...
func AdminPetugasKesehatanUpdate(w http.ResponseWriter, r *http.Request) {
    ...
    var exists int
    var currentIdPengguna, currentIdSkpd, currentNama string
    checkExistQuery := `SELECT COUNT(*), id_pengguna, id_skpd, nama 
        FROM petugas_kesehatan 
        WHERE id = ? AND deleted_date IS NULL 
        GROUP BY id_pengguna, id_skpd, nama`
    err = db.QueryRow(checkExistQuery, req.Id).Scan(&exists, &currentIdPengguna, &currentIdSkpd, &currentNama)
    if err != nil {
    ...
    // Update pengguna table (email and optionally password)
